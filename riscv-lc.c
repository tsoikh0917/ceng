...
void cycle_memory() {
    ...
    if (mio_en) {
        ...
        if (W) {
             ...
             * Lab3-2 assignment
             */
             if (get_MIO_EN(CURRENT_LATCHES.MICROINSTRUCTION)){
                ...
                if(size == -1){
                    ...
                    MEMORY[CURRENT_LATCHES.MAR] = MASK7_0(CURRENT_LATCHES.MDR);
                    MEMORY[CURRENT_LATCHES.MAR] = MASK15_8(CURRENT_LATCHES.MDR);
                }
                else if (size == -3||size==0){
                    MEMORY[CURRENT_LATCHES.MAR] = MASK7_0(CURRENT_LATCHES.MDR);
                    MEMORY[CURRENT_LATCHES.MAR] = MASK15_8(CURRENT_LATCHES.MDR);
                    MEMORY[CURRENT_LATCHES.MAR] = MASK23_16(CURRENT_LATCHES.MDR);
                    MEMORY[CURRENT_LATCHES.MAR] = MASK31_24(CURRENT_LATCHES.MDR);
                }
             ...
             */
             int size = datasize_mux(get_DATASIZE(CURRENT_LATCHES.MICROINSTRUCTION), mask_val(CURRENT_LATCHES.IR, 14, 12),0);
                 if(size == -1){
                    ...
                    MEM_VAL = sext_unit((MEMORY[CURRENT_LATCHES.MAR]+(MEMORY[CURRENT_LATCHES.MAR+1]<<8)+(MEMORY[CURRENT_LATCHES.MAR+2]<<16)+(MEMORY[CURRENT_LATCHES.MAR+3])<<24), 32);
                 }
