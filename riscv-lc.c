        ...
        if (W) {
             ...
             */
             if (get_MIO_EN(CURRENT_LATCHES.MICROINSTRUCTION)) {
                int data = datasize_mux(get_DATASIZE(CURRENT_LATCHES.MICROINSTRUCTION), mask_val(CURRENT_LATCHES.IR, 14, 12), 0);
                switch(data) {
                    case(-1):
             ...
             */
             ...
             switch(data) {
                ...
                case(-2):
                    ...
                    break;
                case(-3): {
                    MEM_VAL = sext_unit((MEMORY[CURRENT_LATCHES.MAR] + (MEMORY[CURRENT_LATCHES.MAR + 1] << 8) +
                                         (MEMORY[CURRENT_LATCHES.MAR + 2] << 16) +
                                         (MEMORY[CURRENT_LATCHES.MAR + 3] << 24)), 32);
                    break;
                }
                default:
