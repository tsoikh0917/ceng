...
void eval_bus_drivers() {
    ...
    value_of_GateMAR = mar_mux(get_MARMUX(CURRENT_LATCHES.MICROINSTRUCTION), value_of_MARMUX, logic_shift_20_function_unit(mask_val(CURRENT_LATCHES.IR, 31, 12)));
    ...
    value_of_shift_function_unit = shift_function_unit(mask_val(CURRENT_LATCHES.IR, 14, 12), mask_val(CURRENT_LATCHES.IR, 31, 25),
            rs1_en(get_RS1En(CURRENT_LATCHES.MICROINSTRUCTION), 0, CURRENT_LATCHES.REGS[mask_val(CURRENT_LATCHES.IR, 19, 15)]),
            rs2_mux(get_RS2MUX(CURRENT_LATCHES.MICROINSTRUCTION), rs2_en(get_RS2En(CURRENT_LATCHES.MICROINSTRUCTION), 0, CURRENT_LATCHES.REGS[mask_val(CURRENT_LATCHES.IR, 24, 20)]), sext_unit(mask_val(CURRENT_LATCHES.IR, 31, 20), 12))
    );
...
void drive_bus() {
    ...
    switch ((_GateMDR << 4) + (_GateRS2 << 3) + (_GatePC << 2) + (_GateALUSHF << 1) + (_GateMAR)) {
        ...
        case 1:
             ...
             *  Lab3-3 assignment
             */
            BUS = value_of_GateMAR; break;
        case 2:
            /*
             *  Lab3-3 assignment
             */
            BUS = value_of_GateALUSHF; break;
        case 4:
            /*
             *  Lab3-3 assignment
             */
            BUS = value_of_GatePC; break;
        case 8:
            /*
             *  Lab3-3 assignment
             */
             BUS = value_of_GateRS2; break;
        case 16:
            /*
             *  Lab3-3 assignment
             */
             BUS = value_of_GateMDR; break;
        default:
             BUS = 0;
